...
<script type="application/ld+json">
{
    ...
    "mainEntity": [
        ...
        {"@@type":"Question","name":"Giá vàng SJC tháng nào tăng mạnh nhất 2021?","acceptedAnswer":{"@@type":"Answer","text":"Xem bảng giá SJC theo tháng với dữ liệu Open/High/Low/Close và phần trăm thay đổi. Biểu đồ trực quan giúp nhận diện tháng tăng/giảm mạnh nhất năm 2021."}},
        {"@@type":"Question","name":"Giá vàng SJC cao nhất năm 2021 bao nhiêu?","acceptedAnswer":{"@@type":"Answer","text":"Giá SJC cao nhất năm 2021 đạt {{ number_format($yearHigh, 2) }} triệu/lượng. Thấp nhất {{ number_format($yearLow, 2) }} triệu. Biên động {{ number_format($yearHigh - $yearLow, 2) }} triệu."}}
    ]
}
</script>
@endpush
...
{{-- Hero --}}
<div class="rounded-sm border border-[#bcbcbc] bg-gradient-to-br from-cyan-50/80 to-white p-4 md:p-6">
    <div class="flex items-center gap-3 mb-3">
        <span class="inline-flex items-center gap-2 rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1 text-sm font-semibold text-cyan-700">{{ $thisYear }}</span>
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5 mb-4">
        ...
        <div class="rounded-sm border-2 border-cyan-300 bg-white p-3 text-center">
            <p class="text-xs font-semibold text-cyan-700 uppercase">Kết thúc năm</p>
            <p class="mt-1 text-lg font-bold text-cyan-900 tabular-nums">{{ number_format($yearClose, 2, ',', '.') }}</p>
        </div>
        <div class="rounded-sm border border-slate-200 bg-white p-3 text-center">
    ...
    <div class="flex flex-wrap gap-2">
        ...
        <a href="/lich-su-gia-vang/gia-vang-{{ $yr }}" class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold no-underline {{ $yr === $thisYear ? 'bg-cyan-100 text-cyan-800' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">{{ $yr }}</a>
        @endforeach
...
<article class="mt-5 rounded-sm border border-[#bcbcbc] bg-white p-4 md:p-6 prose prose-sm max-w-none prose-headings:text-slate-900 prose-p:text-slate-700">
    ...
    <h3>Sự kiện nổi bật ảnh hưởng giá vàng 2021</h3>
    <ul>
        <li><strong>Tháng 1-2:</strong> Vàng khởi đầu năm ở mức cao sau đà tăng kỷ lục 2020. Tuy nhiên, vaccine COVID-19 được triển khai rộng rãi → kỳ vọng phục hồi kinh tế → USD mạnh lên → vàng giảm từ 1,950 về 1,770 USD/oz.</li>
        <li><strong>Tháng 3-6:</strong> Lợi suất trái phiếu Mỹ kỳ hạn 10 năm tăng mạnh từ 1.0% lên 1.75%. "Bond tantrum" gây áp lực lớn lên vàng. XAU/USD sideway quanh 1,750-1,900 USD/oz.</li>
        <li><strong>Tháng 8:</strong> "Flash crash" ngày 09/08 — vàng giảm sốc từ 1,760 xuống 1,680 USD/oz trong vài phút do thanh khoản mỏng phiên châu Á. SJC giảm mạnh theo.</li>
        <li><strong>Tháng 9-11:</strong> FED phát tín hiệu bắt đầu tapering (giảm mua tài sản). Lạm phát Mỹ tăng lên 6.2% (tháng 10) — cao nhất 30 năm. Vàng tăng lại do lo ngại lạm phát.</li>
        <li><strong>Tháng 11-12:</strong> Biến thể Omicron xuất hiện (tháng 11) tạo sóng lo ngại mới. FED chính thức bắt đầu tapering. Vàng kết thúc năm sideway quanh 1,820 USD/oz.</li>
    </ul>

    <h3>Yếu tố chi phối giá vàng 2021</h3>
    <p>Các yếu tố chính ảnh hưởng giá vàng trong năm {{ $thisYear }}:</p>
    <ul>
        <li><strong>Lợi suất trái phiếu Mỹ:</strong> Tăng mạnh từ 0.9% lên 1.75% (Q1) → yếu tố bearish chính. Lãi suất thực tăng làm giảm chi phí cơ hội nắm giữ vàng.</li>
        <li><strong>Lạm phát tăng vọt:</strong> CPI Mỹ từ 1.4% (tháng 1) lên 7.0% (tháng 12). Tranh cãi "transitory" vs "persistent inflation" chi phối thị trường cả năm.</li>
        <li><strong>Vaccine và phục hồi kinh tế:</strong> Triển khai vaccine COVID-19 toàn cầu → risk-on → dòng tiền chảy từ vàng sang cổ phiếu (S&P tăng 27%).</li>
        <li><strong>FED tapering:</strong> Tháng 11 bắt đầu giảm mua $120 tỷ/tháng tài sản. Thị trường đã discount trước → tác động hạn chế lên vàng.</li>
        <li><strong>Kích thích tài khóa:</strong> Gói hạ tầng $1.2 nghìn tỷ và gói chi tiêu xã hội Build Back Better → lo ngại nợ công, lạm phát dài hạn → hỗ trợ vàng.</li>
    </ul>

    <h3>Phân tích kỹ thuật năm 2021</h3>
    <p>Năm 2021 là năm sideway điều chỉnh sau đà tăng bùng nổ 2020. XAU/USD giảm nhẹ ~4% cả năm (1,898 → 1,824 USD/oz). Kênh giá chính là 1,680-1,960 USD/oz. Đường SMA 200 ngày là trục trung tâm — giá xoay quanh đường này cả năm. Flash crash tháng 8 tạo đáy kỹ thuật quan trọng tại 1,680 USD/oz. RSI dao động 35-60 chủ yếu, cho thấy momentum yếu và thiếu xu hướng rõ ràng.</p>
</article>
